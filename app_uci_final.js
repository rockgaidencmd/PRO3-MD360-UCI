...
function calcularPesoIdeal(talla, sexo) {
  // Fórmula rccc.eu: IMC 20.5 × talla(m)²
  // Igual para hombre y mujer (referencia de normopeso)
  const tallaM = talla / 100;
  const pesoIdeal = 20.5 * tallaM * tallaM;
...
function calcularAbdomenAbierto(peso, horas) {
  ...
  const perAbd = Math.round(peso * 1 * horas);
  return perAbd;
}
